...
var handleAudioRequests = function(socket, io){
  socket.on('playAudioRequest', function(){
    io.sockets.emit('playAudioSend');
  });
}

var handleMouseMove = function(socket, io){
  socket.on('moveCursorRequest', function(data){
    io.sockets.emit('moveCursorSend', {
      mouseX: data.mouseX,
      mouseY: data.mouseY,
      guest: socket.id
    })
...
var assignCursor = function(socket, io){
  io.sockets.emit('newCursor', {
    guest: socket.id
  })
}
