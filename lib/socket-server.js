...
var handleBotEvents = function(socket, io, room) {
  socket.on('requestBotOff', function() {
    io.sockets.in(room).emit('botOff', socket.current_bot_data);
    clearInterval(socket.bot_interval);
  });

  socket.on('requestBotOn', function() {
    enableBot(socket, io, room);
  });
}
...
var handleRoomEvents = function(socket, io, room){
  ...
  handleBotEvents(socket, io, room);
...
var roomFull = function(roomname){
  if (roomSize(roomname) >= 4){
    return true;
  }
  return false;
};

var roomSize = function(roomname){
  var rooms = [];
  for(var key in currentRooms) {
    rooms.push(currentRooms[key]);
  }
  return rooms.countInArray(roomname);
}
...
var enableBot = function(socket, io, room){
  var recording = JSON.parse(fs.readFileSync('public/recording.json'));
  var last_audio_row;

  var i = 0;
  socket.bot_interval = setInterval(function(){
    ...
    socket.current_bot_data = {row: rec_data['row'], instrument: rec_data['instrument'], nickname: "bot" };
    if (rec_data["clicked"] && rec_data["row"] > 0) {
      if (last_audio_row === rec_data["row"]) {
        var stop_audio_data = socket.current_bot_data;
        io.sockets.in(room).emit('stopAudioSend', _.extend(socket.current_bot_data, {fromMove: true}));
      }
      io.sockets.in(room).emit('playAudioSend', socket.current_bot_data);
      last_audio_row = rec_data["row"];
    } else {
      io.sockets.in(room).emit('stopAudioSend', _.extend(socket.current_bot_data, {fromMove: false}));
    }
