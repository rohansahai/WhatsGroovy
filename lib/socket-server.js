...
var checkToEnableBot = function(socket, io, room){
  if (roomSize(room) === 1) {
    ...
    socket.bot_interval = setInterval(function(){
      ...
      var audio_data = {row: rec_data['row'], instrument: rec_data['instrument'], nickname: "bot" };
      if (rec_data["clicked"] && rec_data["row"] > 0) {
        if (last_audio_row === rec_data["row"]) {
          var stop_audio_data = audio_data;
          io.sockets.in(room).emit('stopAudioSend', _.extend(audio_data, {fromMove: true}));
        }
        io.sockets.in(room).emit('playAudioSend', audio_data);
        last_audio_row = rec_data["row"];
      } else {
        io.sockets.in(room).emit('stopAudioSend', _.extend(audio_data, {fromMove: false}));
      }
